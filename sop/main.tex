...
At Carleton, I also served as a computational research assistant for the LIGO Scientific Collaboration, did theoretical computer science research with Professor David Liben-Nowell, and wrote two senior theses: an expository piece in which I reproved a few seminal results from algebraic number theory (for my math major) and a paper where I modeled sports games as sequences of discrete events to which I then applied deep-learning techniques originally developed for natural language processing (for my statistics major).
...
For OLS to recover these causal parameters, I require that there is no \emph{dynamic} selection into specific origin-destination pairs.\footnote{
	The reason this differs from the classic age-at-move orthogonality assumption is that to identify time-varying place effects, we require variation in time at move within a fixed cohort, \emph{or} variation in cohort within a fixed time-at-move.
	But within cohort, age at move is colinear with year, and within year of move, age at move is colinear with cohort.
  % There are almost-certainly different potential outcomes between those moving in different years or of different cohorts.
}
...
% \inlinetodo{conclusion of some sort}

% I expect that my research interests will continue to evolve over the course of my Ph.D and would benefit not only from \school's strength in my particular subfields of Economics, but the intellectual environment more broadly.
